...
import {Btn, BtnGhost} from '@app/components/UI-kit';
...
import {TabRoutesEnum} from '@app/navigation';

interface IProps {
  presetId: string;
}

export default function WithPresetUsage({presetId}: IProps) {
  ...
  const use = () => {
    ...
    if (targetDay && targetDay.exercises.length)
      return toastService.error(USE_PRESET_FOR_NOT_EMPTY_DAY_ERROR);
